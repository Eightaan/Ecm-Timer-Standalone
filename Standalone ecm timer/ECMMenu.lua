...
local function CreateMenuFromJson(content, data_table)
    ...
    -- 1.
    Hooks:Add("MenuManagerSetupCustomMenus", "ECM_Timer_v2_Base_SetupCustomMenus_Json_" .. menu_id, function(menu_manager, nodes)
        MenuHelper:NewMenu(menu_id)
    end)
	
    -- 3.
    Hooks:Add("MenuManagerBuildCustomMenus","ECM_Timer_v2_Base_BuildCustomMenus_Json_" .. menu_id, function(menu_manager, nodes)
        local data = {
        ...
        if menu_id == "ECM_Timer_v2_menu" then
            MenuHelper:AddMenuItem(nodes[parent_menu], menu_id, content.title, content.description, menu_priority)
        end
    end)

    -- 2.
    Hooks:Add("MenuManagerPopulateCustomMenus","ECM_Timer_v2_Base_PopulateCustomMenus_Json_" .. menu_id, function(menu_manager, nodes)
        ...
        for k, item in ipairs(items) do
            ...
            elseif i_type == "color" then
                ...
                local data =
                {
                    type = "ECM_Timer_v2MenuItemColor",
                    {
                        _meta = "option",
                        text_id = "ECM_Timer_v2_r",
                        name = "r",
                        value = value.r,
                        localize = true
                    },
                ...
                local params =
                {
                    ...
                    text_id = title,
                    help_id = desc,
                    callback = "ECM_Timer_v2_modify_item_color",
                    localize = localized,
...
function MenuCallbackHandler:ECM_Timer_v2_set_item_value(item)
    ...
    local settings_table = ECM_Timer_v2._data
...
function MenuCallbackHandler:ECM_Timer_v2_save(item)
    ECM_Timer_v2:Save()
end

...
ECM_Timer_v2MenuItemColor = class(CoreMenuItem.Item)
function ECM_Timer_v2MenuItemColor:init(data_node, parameters)
    CoreMenuItem.Item.init(self, data_node, parameters)
    self._type = "ECM_Timer_v2Color"
    self._options = {}
    if data_node then
...
function ECM_Timer_v2MenuItemColor:setup_gui(node, row_item)
...
function ECM_Timer_v2MenuItemColor:_layout(node, row_item)
...
function ECM_Timer_v2MenuItemColor:reload(row_item, node)
...
function ECM_Timer_v2MenuItemColor:highlight_row_item(node, row_item, mouse_over)
    row_item.gui_text:set_color(row_item.color)
    return true
end

function ECM_Timer_v2MenuItemColor:fade_row_item(node, row_item, mouse_over)
    row_item.gui_text:set_color(row_item.color)
    return true
end

---@param value { r: integer, g: integer, b: integer }
function ECM_Timer_v2MenuItemColor:set_value(value)
    for _, option in ipairs(self._options) do
        local o_params = option:parameters()
...
function ECM_Timer_v2MenuItemColor:value()
    local color = {}
    for _, option in ipairs(self._options) do
        local o_params = option:parameters()
...
ECM_Timer_v2MenuSetColorInitiator = class(MenuCrimeNetSpecialInitiator)
function ECM_Timer_v2MenuSetColorInitiator:modify_node(original_node, data)
    return self:setup_node(original_node, data)
end

function ECM_Timer_v2MenuSetColorInitiator:setup_node(node, data)
    node:clean_items()
    data = data or node:parameters().menu_component_data
    if not node:item("divider") then
        self:create_slider(node, {
            name = "red",
            text_id = "ECM_Timer_v2_color_red"
        })
        self:create_slider(node, {
            name = "green",
            text_id = "ECM_Timer_v2_color_green"
        })
        self:create_slider(node, {
            name = "blue",
            text_id = "ECM_Timer_v2_color_blue"
        })
        self:create_divider(node, "divider", nil, 64)
    end
    local params = {
        callback = "ECM_Timer_v2_set_item_color",
        name = "confirm",
    ...
    local params = {
        callback = "ECM_Timer_v2_reset_color_params",
        name = "reset",
        text_id = "ECM_Timer_v2_color_reset",
        align = "right"
    }
    node:add_item(node:create_item({}, params))
...
function ECM_Timer_v2MenuSetColorInitiator:refresh_node(node, data)
    return node
end

function ECM_Timer_v2MenuSetColorInitiator:create_slider(node, params)
    params.callback = "ECM_Timer_v2_update_color"
...
function MenuCallbackHandler:ECM_Timer_v2_update_color()
...
function MenuCallbackHandler:ECM_Timer_v2_reset_color_params()
    local menu = managers.menu:active_menu()
    if not menu then
        return false
    end
...
ECM_Timer_v2MenuNodeCustomizeGadgetGui = ECM_Timer_v2MenuNodeCustomizeGadgetGui or class(MenuNodeGui)
ECM_Timer_v2MenuNodeCustomizeGadgetGui._rec_round_object = MenuNodeCustomizeGadgetGui._rec_round_object
local padding = 10

function ECM_Timer_v2MenuNodeCustomizeGadgetGui:init(node, layer, parameters)
    ...
    ECM_Timer_v2MenuNodeCustomizeGadgetGui.super.init(self, node, layer, parameters)

    self:setup(node)
end

function ECM_Timer_v2MenuNodeCustomizeGadgetGui:setup(node, default_color)
    local r = node:item("red")
    local g = node:item("green")
    local b = node:item("blue")
...
function ECM_Timer_v2MenuNodeCustomizeGadgetGui:make_fine_text(text)
...
function ECM_Timer_v2MenuNodeCustomizeGadgetGui:_setup_item_panel(safe_rect, res)
    ECM_Timer_v2MenuNodeCustomizeGadgetGui.super._setup_item_panel(self, safe_rect, res)
...
function ECM_Timer_v2MenuNodeCustomizeGadgetGui:update_node_colors()
...
function ECM_Timer_v2MenuNodeCustomizeGadgetGui:_setup_item_panel_parent(safe_rect, shape)
    shape = shape or {}
    ...
    ECM_Timer_v2MenuNodeCustomizeGadgetGui.super._setup_item_panel_parent(self, safe_rect, shape)
end

function ECM_Timer_v2MenuNodeCustomizeGadgetGui:reload_item(item)
    ECM_Timer_v2MenuNodeCustomizeGadgetGui.super.reload_item(self, item)
...
function ECM_Timer_v2MenuNodeCustomizeGadgetGui:_align_marker(row_item)
    ECM_Timer_v2MenuNodeCustomizeGadgetGui.super._align_marker(self, row_item)
...
function MenuCallbackHandler:ECM_Timer_v2_modify_item_color(item)
    managers.menu:open_node("ECM_Timer_v2_color_select", {
        {
            item = item
        }
    })
...
function MenuCallbackHandler:ECM_Timer_v2_set_item_color()
    local menu = managers.menu:active_menu()
    ...
    if params.ECM_Timer_v2_callback then
        local callback_args = params.ECM_Timer_v2_callback_arguments
        local fn = ECM_Timer_v2Menu and ECM_Timer_v2Menu[params.ECM_Timer_v2_callback]

        if fn then
            local c = Color(255, color.r, color.g, color.b) / 255
            if type(callback_args) == "table" then
                fn(ECM_Timer_v2Menu, c, unpack(callback_args))
            elseif callback_args ~= nil then
                fn(ECM_Timer_v2Menu, c, callback_args)
            else
                fn(ECM_Timer_v2Menu, c)
            end
        end
    end

    if ECM_Timer_v2.Save then
        ECM_Timer_v2:Save()
    end
